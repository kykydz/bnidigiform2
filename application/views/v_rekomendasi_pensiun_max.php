		...
		<div class="container mt">
			<?php
			 ...
			 foreach($data_bunga->result() as $hitung):
				...
				$max_kredit=($penghasilan*$dsr)/((1/$masa_bulan) + ($bunga/12));
				if($max_kredit>500000000){
					$max=500000000;
				}else{
					$max=$max_kredit;
				}
				$angsuranperbulan=($max / $masa_bulan) + ($max * ($bunga/12));
			...
			<div class="row mt polaroidhasil" style="background-color:<?php echo $warna?>">
				<div class="col-md-3">
					<h5>Maksimal Kredit</h5>
					Rp. <?php echo number_format($max)?>
					<h5>Tenor</h5>
				...
				<script type="text/javascript">
					...
					function myFunction<?php echo $hitung->id_bunga?>(){
						var x = document.getElementById('nasabah<?php echo $hitung->id_bunga?>').value;
						if(x == 'Sudah'){
							document.getElementById('ajukan<?php echo $hitung->id_bunga?>').innerHTML = "<a href='<?php echo $link1 ?>'><button class='btn btn-success btn-aku btn btn-block'>Ajukan Pinjaman</button></a>";
						}else{
							document.getElementById('ajukan<?php echo $hitung->id_bunga?>').innerHTML = "<a href='<?php echo base_url()."home/bukarekening"?>'><button class='btn btn-warning btn-aku btn btn-block' >Buka Rekening</button></a>";
						}
					}
